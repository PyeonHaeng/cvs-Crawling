...
class SaleCrawler:
    ...
    def __crawl_gs_items(self, tag_datas):
        ...
        for tags in tag_datas:
            ...
            num_final = (
                num_bar[0].find_all("a")[-1].text
            )  # 탭별로 0,1,2,3 인덱스를 가져야하지만 사이트가 그냥 다같이 바껴서 암거나 해도 무관
            try:
            ...
            for _ in range(num_final):
                ...
                for g in goods:
                    ...
                    try:
                        data["img"] = self.__check_img_link(
                            g.find("p", class_="img").find("img").get("src")
                        )
                    except:
                        data["img"] = None
                    ...
                    try:
                        data["price"] = self.__covert_price_to_int(
                            g.find("span", class_="cost").text
                        )
                    except:
                        data["price"] = None
        ...
        for tags in tag_datas:
            ...
            for goods in goods_list:
                ...
                for g in goods_boxes:
                    ...
                    try:
                        data["img"] = self.__check_img_link(
                            g.find("img", class_="prod_img").get("src"),
                            ConvName.CU.value,
                        )
                    except:
                        data["img"] = None
                    ...
                    try:
                        data["price"] = self.__covert_price_to_int(
                            g.find("div", class_="price").find("strong").text
                        )
                    except:
                        data["price"] = None
                        print(f'가격찾기 에러 : {data["name"]} - {g}')
    ...
    def __crawl_seven_eleven_items(self, tag_datas):
        ...
        for tags in tag_datas:
            ...
            goods_list = soup.select(tags["prod_list_wrap"])[0].find_all(
                "li", class_=False
            )[1:]
            ...
            for g in goods_list:
                ...
                try:
                    data["img"] = self.__check_img_link(
                        g.find("div", class_="pic_product").find("img").get("src"),
                        ConvName.SevenEleven.value,
                    )
                except:
                    data["img"] = None
                    print(f'그림찾기 에러 : {data["name"]} - {g}')
                ...
                try:
                    data["price"] = self.__covert_price_to_int(
                        g.find("div", class_="price").text.strip()
                    )
                except:
                    data["price"] = None
                    print(f'가격찾기 에러 : {data["name"]} - {g}')
    ...
    def __crawl_mini_stop_items(self, tag_datas):
        ...
        self.driver.switch_to_frame(
            self.driver.find_element_by_xpath("/html/frameset/frame")
        )
        self.driver.switch_to_frame(
            self.driver.find_element_by_xpath("/html/frameset/frame[2]")
        )
        ...
        for tags in tag_datas:
            ...
            while True:
                try:
                    ...
                    prod_count_new = len(
                        soup.select(tags["prod_list_wrap"])[0].find_all("li")
                    )
            ...
            for g in goods_list:
                ...
                try:
                    data["img"] = self.__check_img_link(
                        g.find("img").get("src"), ConvName.MiniStop.value
                    )
                except:
                    data["img"] = None
                    print(f'그림찾기 에러 : {data["name"]} - {g}')
        ...
        for tags in tag_datas:
            ...
            while True:
                try:
                    ...
                    num_bar = soup.select(
                        "body > div.viewContentsWrap > div > div > ul"
                    )[0].find_all("span")
                    num_temp = num_bar[-1].text
                    if (
                        num_final == num_temp
                    ):  # 마지막 페이지 속성갑에 opacity 가 있으면 마지막 페이지
                        break
                    else:
                        num_final = num_temp
                        self.driver.find_element_by_xpath(tags["btn_last_page"]).click()
            ...
            for _ in range(num_final):
                ...
                for g in goods:
                    ...
                    try:
                        data["img"] = self.__check_img_link(
                            g.find("div", class_="itemImg").find("img").get("src"),
                            ConvName.EMart24.value,
                        )
                    except:
                        data["img"] = None
                        print(f'그림찾기 에러 : {data["name"]} - {g}')
